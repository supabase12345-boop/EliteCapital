    ...
    try {
        const { data, error } = await supabaseClient
            ...
            .insert([{
                ...
                duration: packageData.duration || 30, // مدة الباقة
...
// ========== دالة قبول طلب الاشتراك (محدثة) ==========
async function approvePendingPackage(id, adminId) {
    try {
        // 1. جلب بيانات الطلب المعلق
        ...
        // 2. جلب بيانات الباقة الأصلية (لأخذ الربح اليومي والمدة الصحيحين)
        const { data: originalPackage, error: pkgError } = await supabaseClient
            .from('packages')
            .select('daily_profit, duration')
            .eq('id', pending.package_id)
            .single();

        if (pkgError) {
            console.error('تحذير: لم يتم العثور على الباقة الأصلية، سيتم استخدام قيم افتراضية.', pkgError);
            // إذا لم نجد الباقة، نستخدم قيماً افتراضية لمنع تعطل النظام
            var originalPackage = { daily_profit: pending.amount * 0.025, duration: 30 };
        }

        console.log('معالجة طلب:', pending);
        console.log('بيانات الباقة الأصلية:', originalPackage);

        // 3. تحديث حالة الطلب المعلق إلى 'approved'
        await supabaseClient
            .from('pending_packages')
            .update({ 
                status: 'approved',
        ...
        // 4. حساب تواريخ بداية ونهاية الاشتراك باستخدام المدة الصحيحة من الباقة
        const startDate = new Date();
        const endDate = new Date();
        const durationDays = originalPackage?.duration || 30; // نأخذ المدة من الباقة، أو 30 إذا لم توجد
        endDate.setDate(endDate.getDate() + durationDays);
        
        // 5. إنشاء الاشتراك الجديد باستخدام بيانات الباقة الأصلية
        const { data: subscription, error: subError } = await supabaseClient
            ...
            .insert([{
                ...
                // ✅ التعديل الأهم: نستخدم daily_profit من الباقة الأصلية، وليس معادلة ثابتة
                daily_profit: originalPackage.daily_profit,
...
async function processDailyProfits() {
    try {
        ...
        for (const sub of subscriptions || []) {
            ...
            const profitAmount = sub.daily_profit; // استخدام الربح اليومي من الاشتراك
