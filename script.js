...
// ========== دالة حفظ مباشرة باستخدام fetch ==========
async function saveToSupabase(table, data, idField = 'id') {
    try {
        const id = data[idField];
        // محاولة التحديث
        let response = await fetch(`${SUPABASE_URL}/rest/v1/${table}?${idField}=eq.${id}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                'apikey': SUPABASE_ANON_KEY,
        ...
        // إذا فشل التحديث (404 أو غيره)، جرب الإدراج
        if (!response.ok) {
        ...
        if (!response.ok) {
            const errorText = await response.text();
            console.error(`فشل حفظ ${table}:`, errorText);
            return false;
        }
...
async function saveUserToSupabase(user) {
    try {
        ...
        const result = await saveToSupabase('users', userData, 'username');
        if (result) {
            console.log(`✅ تم حفظ المستخدم ${user.username}`);
        }
        return result;
    } catch (error) {
...
async function savePlansToSupabase() {
    try {
        for (let [key, plan] of Object.entries(plans)) {
            ...
            await saveToSupabase('plans', planData);
            
            // حفظ الضريبة
            await saveTaxToSupabase(key, plan.withdrawTax || 0);
        }
        console.log('✅ تم حفظ الباقات');
        return true;
    } catch (error) {
        console.error('❌ خطأ:', error);
        return false;
...
async function saveSettingsToSupabase() {
    try {
        ...
        await saveToSupabase('settings', settingsData);
...
async function saveTaxToSupabase(planId, taxPercent) {
    try {
        ...
        await saveToSupabase('plan_taxes', data, 'plan_id');
        return true;
    } catch (error) {
        console.error(`❌ خطأ:`, error);
        return false;
...
function saveUsers(users) {
    ...
    saveAllUsersToSupabase(users).catch(err => {
        console.error('فشل حفظ المستخدمين:', err);
    });
}
...
function savePlansSettings() {
    localStorage.setItem('plansSettings', JSON.stringify(plans));
    savePlansToSupabase().catch(err => {
        console.error('فشل حفظ الباقات:', err);
    });
}

function savePlanTax(planKey) {
    localStorage.setItem(`tax_${planKey}`, plans[planKey].withdrawTax);
    saveTaxToSupabase(planKey, plans[planKey].withdrawTax).catch(err => {
        console.error(`فشل حفظ ضريبة الباقة ${planKey}:`, err);
    });
}
...
function addSyncButton() {
    ...
    const card = settingsSection.querySelector('.admin-card:last-child');
    if (!card) return;
    
    // التحقق من وجود الزر مسبقاً
    if (card.querySelector('.sync-btn')) return;
    
    const syncBtn = document.createElement('button');
    syncBtn.className = 'admin-save-btn sync-btn';
    syncBtn.style.background = 'linear-gradient(135deg, #10b981, #059669)';
    syncBtn.style.marginTop = '1rem';
    syncBtn.innerHTML = '<i class="fas fa-sync"></i> مزامنة جميع البيانات مع Supabase';
    syncBtn.onclick = async function() {
        ...
        try {
            ...
            showToast('❌ فشل المزامنة: ' + e.message, true);
        }
        this.disabled = false;
        this.innerHTML = '<i class="fas fa-sync"></i> مزامنة جميع البيانات مع Supabase';
    };
...
window.saveAllUsersToSupabase = saveAllUsersToSupabase;
window.savePlansToSupabase = savePlansToSupabase;
