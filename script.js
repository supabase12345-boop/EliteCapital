...
// المتغيرات العامة
// ============================================
const TRC20_WALLET = "TLgdRctbfArVVbQUpTwBy9P55tmozVUW3A";
...
// ============================================
// دوال قراءة الرابط ومعالجة الإحالة
// ============================================
...
function setReferralCodeInRegister(code) {
    if (code) {
        ...
        if (input) {
            ...
            successMsg.id = 'referralCodeSuccess';
            successMsg.className = 'referral-code-auto-filled';
            successMsg.innerHTML = '✅ تم تعبئة كود الإحالة تلقائياً!';
            
            const existingMsg = document.getElementById('referralCodeSuccess');
            if (existingMsg) existingMsg.remove();
            
            input.parentNode.appendChild(successMsg);
            
            setTimeout(() => {
                const msg = document.getElementById('referralCodeSuccess');
...
async function loadAllData() {
    try {
        ...
        if (users && users.length > 0) {
            const usersObj = {};
            users.forEach(u => {
                try { u.subscriptionhistory = JSON.parse(u.subscriptionhistory || '[]'); } catch(e) { u.subscriptionhistory = []; }
                try { u.depositrequests = JSON.parse(u.depositrequests || '[]'); } catch(e) { u.depositrequests = []; }
                try { u.withdrawrequests = JSON.parse(u.withdrawrequests || '[]'); } catch(e) { u.withdrawrequests = []; }
                ...
                usersObj[u.username] = {
                    ...
                    lastProfitClaim: parseInt(u.lastprofitclaim) || 0,
                    subscriptionDate: u.subscriptiondate,
                    subscriptionHistory: u.subscriptionhistory,
                    depositRequests: u.depositrequests,
                    withdrawRequests: u.withdrawrequests,
                    referralCode: u.referralcode,
                    referredBy: u.referredby,
                    referralBonus: parseFloat(u.referralbonus) || 0,
                    referredUsers: u.referredusers,
                    referralBonusGiven: u.referralbonusgiven,
                    restrictWithdraw: u.restrictwithdraw,
                    restrictProfit: u.restrictprofit,
                    createdAt: u.createdat,
                    transferHistory: u.transferhistory
                };
            });
...
async function getUsers() {
    ...
    await loadAllData();
    return JSON.parse(localStorage.getItem('investUsers') || '{}');
}
        ...
        registerBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (saved) {
                    ...
                    return showToast('❌ فشل حفظ البيانات في السيرفر. حاول مرة أخرى', true);
                }
...
async function processReferral(newUsername, referralCode) {
    try {
        ...
        for (let [uname, u] of Object.entries(users)) {
        ...
        if (referrer) {
            users[newUsername].referredBy = referrerUsername;
...
function loadUserPanel() {
    ...
    document.querySelectorAll('.nav-item').forEach(item => {
        ...
        newItem.addEventListener('click', () => {
            const pageId = newItem.dataset.page;
            document.querySelectorAll('#userPages .page').forEach(p => p.classList.remove('active'));
    ...
    if (collectBtn) {
        ...
        newCollectBtn.addEventListener('click', collectProfit);
    }
    ...
    // تحديث دالة نسخ الإحالة لتشمل الرابط الكامل
    updateReferralCodeCopy();
    
    const copyWallet = document.getElementById('copyWalletAddress');
    if (copyWallet) {
        const newCopyWallet = copyWallet.cloneNode(true);
        copyWallet.parentNode.replaceChild(newCopyWallet, copyWallet);
        newCopyWallet.addEventListener('click', () => { 
            const addressText = document.getElementById('trc20Address')?.innerText || TRC20_WALLET;
    ...
    if (submitDeposit) {
        const newSubmitDeposit = submitDeposit.cloneNode(true);
        submitDeposit.parentNode.replaceChild(newSubmitDeposit, submitDeposit);
        newSubmitDeposit.addEventListener('click', submitDepositRequest);
    }
    
    const submitWithdraw = document.getElementById('submitWithdrawRequest');
    if (submitWithdraw) {
        const newSubmitWithdraw = submitWithdraw.cloneNode(true);
        submitWithdraw.parentNode.replaceChild(newSubmitWithdraw, submitWithdraw);
        newSubmitWithdraw.addEventListener('click', submitWithdrawRequest);
    }
    
    const submitTransfer = document.getElementById('submitTransfer');
    if (submitTransfer) {
        const newSubmitTransfer = submitTransfer.cloneNode(true);
        submitTransfer.parentNode.replaceChild(newSubmitTransfer, submitTransfer);
        newSubmitTransfer.addEventListener('click', transferBalance);
    }
    
    const transferAmountInput = document.getElementById('transferAmount');
    if (transferAmountInput) transferAmountInput.addEventListener('input', calculateTransferDetails);
    
    const transferRecipientInput = document.getElementById('transferRecipientCode');
    if (transferRecipientInput) transferRecipientInput.addEventListener('input', calculateTransferDetails);
}

// دالة نسخ كود الإحالة مع الرابط الكامل
function updateReferralCodeCopy() {
    ...
    newCopyBtn.addEventListener('click', () => {
        if (currentUser && currentUser.referralCode) {
            ...
            const fullLink = `${baseUrl}?ref=${currentUser.referralCode}`;
...
function renderPlans() {
    ...
    for (let [key, plan] of Object.entries(plans)) {
        ...
        card.innerHTML = `
            <div class="plan-info">
                <h3><i class="${plan.icon}" style="color:${plan.color}"></i> ${plan.name}</h3>
                <div class="plan-price"><span class="price-value"><i class="fas fa-tag"></i> ${plan.price} $</span></div>
                <div class="plan-profit"><span class="profit-value"><i class="fas fa-chart-line"></i> الربح اليومي: ${plan.dailyProfit} $</span></div>
                <div class="plan-roi">عائد: ${((plan.dailyProfit/plan.price)*100).toFixed(1)}% يومياً</div>
                ${taxDisplay}
            </div>
            <button id="buy-${key}" ${currentUser && currentUser.activePlan ? 'disabled' : ''}><i class="fas fa-shopping-cart"></i> اشتراك</button>
        `;
...
let isProcessingProfit = false;
async function collectProfit() {
    if (isProcessingProfit) return;
    ...
    try {
        isProcessingProfit = true;
        if (btn) btn.disabled = true;

    const now = Date.now();
    const lastClaim = currentUser.lastProfitClaim || 0;
    if ((now - lastClaim) >= (24 * 3600 * 1000)) {
        ...
        console.error(error);
        showToast('حدث خطأ أثناء جني الأرباح', true);
    } finally {
        isProcessingProfit = false;
        if (btn) btn.disabled = false;
    }
}

async function checkAndGiveReferralBonus(username) {
    const users = await getUsers();
    const user = users[username];
...
async function submitWithdrawRequest() {
    ...
    const btn = document.querySelector('button[onclick="submitWithdrawRequest()"]') || document.getElementById('withdrawBtn');
    ...
    updateWithdrawPage();
        ...
        console.error(error);
        showToast('حدث خطأ أثناء تقديم طلب السحب', true);
    } finally {
...
function updateSubscriptionPage() {
    ...
    if (!currentUser || !currentUser.activePlan) { 
        ...
        historyEl.innerHTML = '<div>لا يوجد سجل</div>';
        return; 
    }
...
function calculateTransferDetails() {
    ...
    const amount = parseFloat(amountInput?.value);
    const recipientCode = recipientInput?.value.trim();
...
function loadAdminPanel() {
    ...
    document.querySelectorAll('.admin-nav-btn').forEach(btn => {
        ...
        newBtn.addEventListener('click', () => {
            ...
            const section = document.getElementById(`admin${capitalize(newBtn.dataset.adminSection)}Section`);
    ...
    if (addPlanBtn) {
        ...
        newAddPlanBtn.addEventListener('click', () => openAddPlanModal());
    }
    
    const saveBonus = document.getElementById('saveReferralBonus');
    if (saveBonus) {
        const newSaveBonus = saveBonus.cloneNode(true);
        saveBonus.parentNode.replaceChild(newSaveBonus, saveBonus);
        newSaveBonus.addEventListener('click', saveReferralBonusSettings);
    }
    
    const saveTax = document.getElementById('saveTaxSettings');
    if (saveTax) {
        const newSaveTax = saveTax.cloneNode(true);
        saveTax.parentNode.replaceChild(newSaveTax, saveTax);
        newSaveTax.addEventListener('click', saveTaxSettings);
    }
    
    const saveAlert = document.getElementById('saveAlertSettings');
    if (saveAlert) {
        const newSaveAlert = saveAlert.cloneNode(true);
        saveAlert.parentNode.replaceChild(newSaveAlert, saveAlert);
        newSaveAlert.addEventListener('click', saveAlertSettings);
    }
}
...
function renderUsersList(search = '') {
    ...
    Object.entries(users).forEach(([username, user]) => {
        ...
        const planName = user.activePlan ? (plans[user.activePlan]?.name || 'باقة غير معروفة') : 'لا توجد';
    ...
    for (let [key, plan] of Object.entries(plans)) {
        ...
        div.innerHTML = `
            <div class="plan-info">
                ...
                <small>السعر: ${plan.price}$ | الربح: ${plan.dailyProfit}$ | الضريبة: ${plan.withdrawTax || 0}%</small>
            </div>
            <div class="plan-actions">
                <button class="btn-edit-plan" onclick="openEditPlanModal('${key}')"><i class="fas fa-edit"></i></button>
                <button class="btn-delete-plan" onclick="deletePlan('${key}')"><i class="fas fa-trash"></i></button>
            </div>
        `;
        container.appendChild(div);
    }
...
async function savePlansToSupabase() {
    ...
    for (let [key, plan] of Object.entries(plans)) {
        try {
            ...
            if (existing && existing.length > 0) {
                await supabaseRequest(`plans?id=eq.${plan.id}`, 'PATCH', planData);
            } else {
            ...
            const existingTax = await supabaseRequest(`plan_taxes?plan_id=eq.${key}`);
            if (existingTax && existingTax.length > 0) {
                await supabaseRequest(`plan_taxes?plan_id=eq.${key}`, 'PATCH', taxData);
            } else {
                await supabaseRequest('plan_taxes', 'POST', taxData);
            }
        } catch (e) {
...
function loadSettingsToAdmin() {
    ...
    const alertBtnText = document.getElementById('adminAlertButtonText');
    if (alertBtnText) alertBtnText.value = globalAlert.buttonText || '';
    
    const alertBg = document.getElementById('adminAlertBgColor');
    if (alertBg) alertBg.value = globalAlert.bgColor || '#fef3c7';
...
function openUpgradePlanModal(username) {
    ...
    document.getElementById('confirmUpgradePlan').onclick = async () => {
        ...
        const users2 = await getUsers();
        users2[username].activePlan = newPlanKey;
        users2[username].lastProfitClaim = Date.now();
        users2[username].subscriptionDate = Date.now();
        if (!users2[username].subscriptionHistory) users2[username].subscriptionHistory = [];
        users2[username].subscriptionHistory.unshift({ 
            planName: plans[newPlanKey].name, 
            price: plans[newPlanKey].price, 
            date: Date.now(), 
            type: 'ترقية' 
        });
        
        await saveUser(users2[username]);
        localStorage.setItem('investUsers', JSON.stringify(users2));
...
async function initApp() {
    try {
        ...
        if (!success) {
            console.warn('⚠️ فشل تحميل بعض البيانات من Supabase، سيتم الاعتماد على البيانات المحلية إن وجدت');
        }
        ...
        if (refCode) {
            setTimeout(() => {
                setReferralCodeInRegister(refCode);
