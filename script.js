...
async function saveAllUsersToSupabase(users) {
    try {
        const promises = Object.values(users).map(user => {
            return supabaseRequest(`users?username=eq.${user.username}`, 'PATCH', user).catch(() => {
                return supabaseRequest('users', 'POST', user);
            });
        });
...
async function savePlansToSupabase() {
    try {
        const promises = Object.values(plans).map(plan => {
            return supabaseRequest(`plans?id=eq.${plan.id}`, 'PATCH', plan).catch(() => {
                return supabaseRequest('plans', 'POST', plan);
            });
        });
        await Promise.all(promises);
        return true;
...
async function saveTaxToSupabase(planId, taxPercent) {
    try {
        ...
        await supabaseRequest(`plan_taxes?plan_id=eq.${planId}`, 'PATCH', data).catch(() => {
            return supabaseRequest('plan_taxes', 'POST', data);
        });
        return true;
    } catch (error) {
...
function loadUsers() {
    ...
    const defaultUsers = {
        admin: { 
            ...
            restrictWithdraw: false, restrictProfit: false, createdAt: Date.now()
        }
    };
...
function submitDepositRequest() {
    ...
    document.getElementById('depositAmountRequest').value = ''; 
...
function submitWithdrawRequest() {
    ...
    document.getElementById('withdrawAmountRequest').value = ''; 
...
function updateAdminStats() {
    ...
    document.getElementById('adminTotalInvested').innerText = totalInvested;
