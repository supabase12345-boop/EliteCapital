...
async function loadAllData() {
    try {
        ...
        if (users && users.length > 0) {
            const usersObj = {};
            users.forEach(u => {
                // تحويل السلاسل النصية JSON إلى كائنات
...
function loadUserPanel() {
    ...
    if (copyReferral) {
        const newCopyReferral = copyReferral.cloneNode(true);
        copyReferral.parentNode.replaceChild(newCopyReferral, copyReferral);
        newCopyReferral.addEventListener('click', () => { 
            if (currentUser) {
